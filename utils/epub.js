...
try {
    ...
    ZipFile.prototype.readFile = function(name, cb) {
        this.admZip.readFileAsync(this.admZip.getEntry(name),
...
class EPub extends EventEmitter {
    ...
    getRootFiles() {
        ...
        this.zip.readFile(this.containerFile, (function(err, data) {
            ...
            xmlparser.on('end', (function(result) {
                ...
                if (Array.isArray(rootfile)) {
                    ...
                    for (i = 0, len = rootfile.length; i < len; i++) {
                        if (rootfile[i]['@']['media-type'] &&
                            ...
                            filename = rootfile[i]['@']['full-path'].toLowerCase().
                                trim();
                            break;
                        }
                    }
                    ...
                    if (rootfile['@']['media-type'] !=
                        'application/oebps-package+xml' ||
                        !rootfile['@']['full-path']) {
                        this.emit('error',
                            new Error('Rootfile in unknown format'));
                        return;
                    }
                ...
                if (!this.rootFile) {
                    this.emit('error',
                        new Error('Rootfile not found from archive'));
                    return;
                }
    ...
    handleRootFile() {
        ...
        this.zip.readFile(this.rootFile, (function(err, data) {
            ...
            xmlparser.on('error', (function(err) {
                this.emit('error', new Error(
                    'Parsing container XML failed in handleRootFile: ' +
                    err.message));
                return;
            }).bind(this));
                ...
                case 'language':
                    if (Array.isArray(metadata[keys[i]])) {
                        this.metadata.language = String(
                            metadata[keys[i]][0] && metadata[keys[i]][0]['#'] ||
                            metadata[keys[i]][0] || '').toLowerCase().trim();
                    } else {
                        this.metadata.language = String(
                            metadata[keys[i]]['#'] || metadata[keys[i]] || '').
                            toLowerCase().
                ...
                case 'identifier':
                    if (metadata[keys[i]]['@'] &&
                        ...
                        this.metadata.ISBN = String(
                            metadata[keys[i]]['#'] || '').trim();
                    } else if (metadata[keys[i]]['@'] &&
                        metadata[keys[i]]['@'].id &&
                        metadata[keys[i]]['@'].id.match(/uuid/i)) {
                        this.metadata.UUID = String(
                            metadata[keys[i]]['#'] || '').
                            replace('urn:uuid:', '').
                            toUpperCase().
                            trim();
                        ...
                        for (j = 0; j < metadata[keys[i]].length; j++) {
                            if (metadata[keys[i]][j]['@']) {
                                if (metadata[keys[i]][j]['@']['opf:scheme'] ==
                                    'ISBN') {
                                    this.metadata.ISBN = String(
                                        metadata[keys[i]][j]['#'] || '').trim();
                                } else if (metadata[keys[i]][j]['@'].id &&
                                    metadata[keys[i]][j]['@'].id.match(
                                        /uuid/i)) {
                                    this.metadata.UUID = String(
                                        metadata[keys[i]][j]['#'] || '').
                                        replace('urn:uuid:', '').
    ...
    parseTOC() {
        ...
        this.zip.readFile(this.spine.toc.href, (function(err, data) {
            ...
            xmlparser.on('error', (function(err) {
                this.emit('error', new Error(
                    'Parsing container XML failed in TOC: ' + err.message));
                return;
            }).bind(this));
    ...
    walkNavMap(branch, path, id_list, level) {
        ...
        for (var i = 0; i < branch.length; i++) {
            if (branch[i].navLabel) {
                ...
                var title = '';
                if (branch[i].navLabel && typeof branch[i].navLabel.text ==
                    'string') {
                    title = branch[i].navLabel && branch[i].navLabel.text ||
                    branch[i].navLabel === branch[i].navLabel &&
                    branch[i].navLabel.text.length > 0 ?
                        (branch[i].navLabel && branch[i].navLabel.text ||
                            branch[i].navLabel || '').trim() : '';
                }
                var order = Number(
                    branch[i]['@'] && branch[i]['@'].playOrder || 0);
                if (isNaN(order)) {
                    order = 0;
                }
                var href = '';
                if (branch[i].content && branch[i].content['@'] &&
                ...
                if (href) {
                    ...
                    if (id_list[element.href]) {
                        ...
                        // use new one
                        element.href = href;
                        element.id = (branch[i]['@'] && branch[i]['@'].id ||
                            '').trim();
                    }
            ...
            if (branch[i].navPoint) {
                output = output.concat(
                    this.walkNavMap(branch[i].navPoint, path, id_list,
                        level + 1));
            }
        }
        return output;
    ...
    getChapter(id, callback) {
        this.getChapterRaw(id, (function(err, str) {
            ...
            str = str.replace(/(\s)(on\w+)(\s*=\s*["']?[^"'\s>]*?["'\s>])/g,
                function(o, a, b, c) {
                    return a + 'skip-' + b + c;
                });

            // replace images
            str = str.replace(/(\ssrc\s*=\s*["']?)([^"'\s>]*?)(["'\s>])/g,
                (function(o, a, b, c) {
            ...
            // replace links
            str = str.replace(/(\shref\s*=\s*["']?)([^"'\s>]*?)(["'\s>])/g,
                (function(o, a, b, c) {
                    var linkparts = b && b.split('#');
                    var link = linkparts.length ? path.concat(
                        [(linkparts.shift() || '')]).join('/').trim() : '',
                        element;
    ...
    getChapterRaw(id, callback) {
        if (this.manifest[id]) {
            ...
            if (!(this.manifest[id]['media-type'] == 'application/xhtml+xml' ||
    ...
    getImage(id, callback) {
        if (this.manifest[id]) {
            ...
            if ((this.manifest[id]['media-type'] || '').toLowerCase().
                trim().
                substr(0, 6) != 'image/') {
